...
parser.add_argument(action='store',dest='coding',nargs='+',help='The name of the reference fasta that contains just the coding sequences')
parser.add_argument(action='store',dest='ref',nargs='+',help='The name of the reference csv that contains the concated positions')
parser.add_argument(action='store',dest='muscle',nargs='+',help='path to muscle exicutable')
...
with open('./fasta.config.groovy','w') as config:
    ...
    config.write('MUSCLE_DIR='+ '\"'+muscle+ '\"'+'\n')
