...
```{r}
ggplot(var.2007.8.df,aes(x=freq.var))+geom_histogram(color="white",binwidth = 0.01)#+scale_y_log10() + ggtitle("All samples")
ggplot(qual.2007.8,aes(x=freq.var))+geom_histogram(color="white",binwidth = 0.01)#+scale_y_log10() + ggtitle("Only high quality samples")
...
ggplot(data=all.data.isnv.day,aes(y=iSNV,x=as.factor(Day.of.Infection.sample.collected)))+geom_dotplot(stackdir = "center",binaxis = 'y',position='dodge',dotsize = 0.5)+ylab("Number of SNV")+ggtitle("iSNV by day")+xlab("Day of Infection")+facet_wrap(~data)
```

```{r,by_day.densisty}
ggplot(data=all.data.isnv.day,aes(x=as.factor(Day.of.Infection.sample.collected),y=iSNV,fill=Intervention))+geom_boxplot()+ylab("Number of SNV")+ylab("Number of SNV")+ggtitle("iSNV by day")+xlab("Day of Infection")+facet_wrap(~data)
...
data<-subset(df,variable==var)
means<-ddply(data,~quality+data,summarize,iSNV=mean(iSNV))
out<-ggplot(data=data,aes(y=iSNV,x=quality))+geom_dotplot(stackdir = "center",binaxis = 'y',dotsize = 0.5)+ylab("Number of SNV")+ggtitle(title)+xlab(var)+facet_wrap(~data)
out<-out+geom_point(data=means,aes(y=iSNV,x=quality),shape=95,size=20,col='red')+facet_wrap(~data)
print(out)
}
