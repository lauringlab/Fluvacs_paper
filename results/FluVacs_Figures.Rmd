...
```{r}
qual.ha.snv<-ddply(minor.07.08,~HAI.cut+Id,summarize,HAI=ifelse(sum(HAI.cut)==0,yes = "<40",no= "\u2265 40"),iSNV=sum(which(chr=="HA")))
...
fig_3C<-ggplot(data=qual.ha.snv,aes(y=iSNV,x=HAI))+geom_dotplot(stackdir = "center",binaxis = 'y',binwidth = 1,dotsize = 0.15)+ylab("Number of iSNV")+stat_summary(fun.data="plot.median", geom="errorbar", colour="black", width=0.95, size=0.5)
fig_3C
...
qual.na.snv<-ddply(minor.07.08,~NAI.cut+Id,summarize,NAI=ifelse(sum(NAI.cut)==0,yes = "<40",no= "\u2265 40"),iSNV=sum(which(chr =="NR")))
qual.na.snv<-subset(qual.na.snv,!(is.na(NAI)))

fig_3D<-ggplot(data=qual.na.snv,aes(y=iSNV,x=NAI))+geom_dotplot(stackdir = "center",binaxis = 'y',binwidth = 1,dotsize = 0.15)+stat_summary(fun.data="plot.median", geom="errorbar", colour="black", width=0.95, size=0.5)+ylab("Number of iSNV")
fig_3D
...
```{r,extra_iSNV}
qual.ha.snv<-ddply(minor.07.08,~HAI.cut+Id,summarize,HAI=ifelse(sum(HAI.cut)==0,yes = "<40",no= "\u2265 40"),iSNV=length(mutation))

sup_5A<-ggplot(data=qual.ha.snv,aes(y=iSNV,x=HAI))+geom_dotplot(stackdir = "center",binaxis = 'y',binwidth = 1,dotsize = 0.3)+stat_summary(fun.data="plot.median", geom="errorbar", colour="black", width=0.5, size=0.5)+ylab("Number of iSNV")
sup_5A


qual.na.snv<-ddply(minor.07.08,~NAI.cut+Id,summarize,NAI=ifelse(sum(NAI.cut)==0,yes = "<40",no= "\u2265 40"),iSNV=length(mutation))
qual.na.snv<-subset(qual.na.snv,!(is.na(NAI)))

sup_5B<-ggplot(data=qual.na.snv,aes(y=iSNV,x=NAI))+geom_dotplot(stackdir = "center",binaxis = 'y',binwidth = 1,dotsize = 0.3)+stat_summary(fun.data="plot.median", geom="errorbar", colour="black", width=0.5, size=0.5)+ylab("Number of iSNV")
sup_5B
...
## Supplemental figure 6 2004-2005 & 2005-2006
...
sup_6A<-ggplot(minor.04.05,aes(x=freq.var))+geom_histogram(color="white",binwidth = 0.01)+xlab("Frequency")+ylab("Number of iSNV")+scale_x_continuous(breaks=seq(0,.5,0.1),labels=c("0%","10%","20%","30%","40%","50%"))
sup_6A+ggtitle("2004-2005")
...
sup_6B<-ggplot(minor_counts,aes(x=count))+geom_histogram(color="white",binwidth=1)+xlab("Samples with iSNV")+ylab("Number of iSNV")
sup_6B+ggtitle("2004-2005")
...
sup_6C<-ggplot(minor.05.06,aes(x=freq.var))+geom_histogram(color="white",binwidth = 0.01)+xlab("Frequency")+ylab("Number of iSNV")+scale_x_continuous(breaks=seq(0,.5,0.1),labels=c("0%","10%","20%","30%","40%","50%"))
sup_6C+ggtitle("2005-2006")

pdf("./figures/sup_6C.pdf")
sup_6C
dev.off()
...
sup_6D<-ggplot(minor_counts,aes(x=count))+geom_histogram(color="white",binwidth=1)+xlab("Samples with iSNV")+ylab("Number of iSNV")
sup_6D+ggtitle("2005-2006")



pdf("./figures/sup_6D.pdf")
sup_6D
dev.off()
```
