...
```{r}
ggplot(subset(all.df,freq.var<0.9),aes(x=freq.var))+geom_histogram(color="white")+scale_x_log10()
ggplot(subset(all.qual.df,freq.var<0.9),aes(x=freq.var))+geom_histogram(color="white")+scale_x_log10()
...
```{r}
#ggplot(data=subset(all.snv,!is.na(Vax)),aes(x=V1))+geom_boxplot()
```
#### High quality

```{r}
#ggplot(data=subset(qual.snv,!is.na(Vax)),aes(x=V1))+geom_boxplot()
```
...
#ggplot(data=subset(all.snv,!is.na(Vax)),aes(x=V1,color=Vax))+geom_boxplot()
```
#### High quality

```{r}
qual.snv<-count_muts(subset(qual.df,chr %in%c("HA","N_A","NR")))

#ggplot(data=subset(qual.snv,!is.na(Vax)),aes(x=V1,color=Vax))+geom_boxplot()
```
...
ggplot(subset(all.qual.df,antigenic==T),aes(x=freq.var))+geom_histogram()
```
