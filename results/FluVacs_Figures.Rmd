...
```
There were `r removed` samples removed from the the minority analysis based on a mean coverage less than 1000X.
...
```{r}
qual.ha.snv<-ddply(minor.07.08,~HAI.post.vax+Id,summarize,HAI=HAI.post.vax[1],iSNV=sum(which(chr=="HA")))
...
titer_3C<-ggplot(data=qual.ha.snv,aes(y=iSNV,x=as.factor(HAI)))+geom_dotplot(stackdir = "center",binaxis = 'y',binwidth = 1,dotsize = 0.15)+ylab("Number of iSNV")+stat_summary(fun.data="plot.median", geom="errorbar", colour="black", width=0.95, size=0.5)+xlab("HAI")+scale_x_discrete(labels=paste0("1:",unique(qual.ha.snv$HAI)))+scale_y_continuous(breaks=c(0,2,4,6))
titer_3C

pdf("./figures/titer_3C.pdf",width = 10)
titer_3C
dev.off()

qual.na.snv<-ddply(minor.07.08,~NAI.post.vax+Id,summarize,NAI=NAI.post.vax[1],iSNV=sum(which(chr =="NR")))
qual.na.snv<-subset(qual.na.snv,!(is.na(NAI)))

titer_3D<-ggplot(data=qual.na.snv,aes(y=iSNV,x=as.factor(NAI)))+geom_dotplot(stackdir = "center",binaxis = 'y',binwidth = 1,dotsize = 0.15)+stat_summary(fun.data="plot.median", geom="errorbar", colour="black", width=0.95, size=0.5)+ylab("Number of iSNV")+xlab("NAI")+scale_x_discrete(labels=paste0("1:",unique(qual.na.snv$NAI)))+scale_y_continuous(breaks=c(0,2,4,6,8,10))
titer_3D

pdf("./figures/titer_3D.pdf",width=12)
titer_3D
dev.off()
```
